...
*Setup instructions for a gitian build of Murraycoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Murraycoin
...
- [Building Murraycoin](#building-murraycoin)
...
Clone the git repositories for murraycoin and gitian,

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/murraycoin/murraycoin
```
...
In [doc/release-process.md](release-process.md) in the murraycoin repository under 'Fetch and build inputs'.
...
To build Murraycoin (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the murraycoin repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/murraycoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/murraycoin/murraycoin
...
For example:
```bash
URL=https://github.com/langerhans/murraycoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit murraycoin=${COMMIT} --url murraycoin=${URL} ../murraycoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit murraycoin=${COMMIT} --url murraycoin=${URL} ../murraycoin/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/murraycoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/murraycoin-build.assert
```
...
[murraycoin/gitian.sigs](https://github.com/murraycoin/gitian.sigs/) repository, or
