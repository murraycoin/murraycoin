...
#if defined(NDEBUG)
# error "Murraycoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Murraycoin Signed Message:\n";
...
//TODO: this isn't identical to murraycoin reference client.
...
static const int64_t nMurrayFork = 130000; // Patch effective @ block 130,000

int64_t GetBlockValue(int nHeight, int64_t nFees)
{
   int64_t nSubsidy = 60 * COIN;
	  int BlockCountA = 5000;
      int BlockCountB = 300000;
      if (nHeight >= BlockCountA * 22) {
		nSubsidy >>= ((nHeight - (BlockCountA * 22)) / (BlockCountB)); // Subsidy is cut in half every 300,000 blocks (~1 year)
	  }	
	  else if (nHeight >= BlockCountA * 6) { nSubsidy = 75 * COIN; }
      else if (nHeight >= BlockCountA * 5) { nSubsidy = 80 * COIN; }
	  else if (nHeight >= BlockCountA * 4) { nSubsidy = 130 * COIN; }
	  else if (nHeight >= BlockCountA * 3) { nSubsidy = 210 * COIN; }
	  else if (nHeight >= BlockCountA * 2) { nSubsidy = 320 * COIN; }
	  else if (nHeight >= BlockCountA) { nSubsidy = 500 * COIN; }
	  else { nSubsidy = 1950 * COIN; }

   return nSubsidy + nFees;
}

static const int64_t nTargetTimespan = 60 * 60; // Murraycoin: 1 hour
static const int64_t nTargetTimespanNEW = 100; // Murraycoin: 100 seconds
static const int64_t nTargetSpacing = 100; // Murraycoin: 100 seconds
static const int64_t nInterval = nTargetTimespan / nTargetSpacing; // Pre-fork: Every 36 blocks; Post-fork: Every block
...
unsigned int ComputeMinWork(unsigned int nBase, int64_t nTime)
{
    ...
    {
        if(chainActive.Height()+1 < nMurrayFork){
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    bool fNewDifficultyProtocol = (nHeight >= nMurrayFork);
    ...
    // Murraycoin: This fixes an issue where a 51% attack can change difficulty at will.
    ...
    else
    {
       ...
       int nActualTimespanMax = retargetTimespan*5/3;
       int nActualTimespanMin = retargetTimespan*3/5;
...
void ThreadScriptCheck() {
    RenameThread("murraycoin-scriptch");
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // BIP16 was always active in Murraycoin
...
{
    // Murraycoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
